...
ones into patches/kernel-org-7.1/sauce/ with a generated sauce.series for
human review and curation before committing.
"""
...
SAUCE_DIR  = PATCH_DIR / "kernel-org-7.1" / "sauce"
...
def cmd_sauce(tree: Path, dry_run: bool = False) -> int:
    """Discover which Ubuntu 7.0 SAUCE patches apply cleanly to a 7.1.x tree."""
    ...
    with tempfile.TemporaryDirectory(prefix="hyphaed-sauce-") as td:
        ...
        sauce_series.write_text(
            ...
            "# Review and curate before appending to patches/kernel-org-7.1/series.\n"
            "# Each entry here is CLEAN against the 7.1.x tree — but check:\n"
        ...
        print(
            ...
            f"then append chosen entries to patches/kernel-org-7.1/series\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument(
        ...
        help="SAUCE discovery: apt source Ubuntu linux, eval, copy CLEAN patches to "
             "patches/kernel-org-7.1/sauce/ for review",
    )
    ap.add_argument(
    ...
    if args.sauce:
        return cmd_sauce(tree, dry_run=args.dry_run)
